...
def delete_item
    respond_to do |format|
      ...
      if @list.save
        format.html { render :edit, notice: (item + ' - succesffully removed!') }
        format.json { render :show, status: :ok, location: @list }
      else
        format.html { render :edit }
        format.json { render json: @list.errors, status: :unprocessable_entity }
  ...
  private
    ...
    def confirm_owner
      redirect_to lists_url, notice: "You are not allowed to access that list" if current_user.id != @list.user_id
    end
end